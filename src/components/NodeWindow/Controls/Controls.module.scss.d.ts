export declare const arrayContainer: string;
export declare const arrayIndex: string;
export declare const arrayTitle: string;
export declare const arrayTitleInvalid: string;
export declare const boolCheckbox: string;
export declare const boolCheckboxContainer: string;
export declare const controlContainer: string;
export declare const controlIndex: string;
export declare const controlLabelEditable: string;
export declare const controlLabelEditableInvalid: string;
export declare const controlLabelUneditable: string;
export declare const controlSeparator: string;
export declare const deleteControlButton: string;
export declare const generalControl: string;
export declare const nodeControlContainer: string;
export declare const nodeDragSelector: string;
export declare const nodeDragSelectorEmpty: string;
export declare const nodeDragSelectorFull: string;
export declare const nodeLabel: string;
export declare const numberField: string;
export declare const textField: string;
export declare const textFieldInvalid: string;
export declare const textFieldUneditable: string;
